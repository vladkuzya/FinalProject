	...
	<div class="head">
		...
		<div class="content">
			...
			<fieldset id="registration_form">
				<legend>
					<fmt:message key="signup_jsp.legend.registration" />
				</legend>
				<form action="/FinalProject/controller" method="post">
					<input type="hidden" name="command" value="registration" />
					<p>
						<fmt:message key="signup_jsp.label.login" />
						<input name="login" maxlength="20" pattern="[A-Za-z0-9]{4,}"
							title="<fmt:message key="title.login"/>" required><br />
					</p>
					<p>
						<fmt:message key="signup_jsp.label.password" />
						<input type="password" name="password" maxlength="20"
							pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"
							title="<fmt:message key="title.password"/>" required><br />
					</p>
					<p>
						<fmt:message key="signup_jsp.label.name" />
						<input name="first_name" maxlength="20"
							pattern="[A-Za-zА-Яа-яЁё]{2,}"
							title="<fmt:message key="title.first_name"/>" required><br />
					</p>
					<p>
						<fmt:message key="signup_jsp.label.surname" />
						<input name="last_name" maxlength="20"
							pattern="[A-Za-zА-Яа-яЁё]{2,}"
							title="<fmt:message key="title.last_name"/>" required><br />
					</p>
					<p>
					...
					<p>
						<input type="submit"
							value="<fmt:message key="signup_jsp.form.submit_signup"/>">
				</form>
			</fieldset>
		</div>
	</div>
